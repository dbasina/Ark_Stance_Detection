...
def get_args_parser():
    ...
    parser.add_option("--model", dest="model_name", help="stance_bert_masked | stance_bert_unmasked", default="stance_bert_masked", type="string")
    ...
    parser.add_option("--data_set", dest="dataset_list", help="unMaskedABSA_race",  action="append")
    ...
    parser.add_option("--pretrain_epochs", dest="pretrain_epochs", help="num of omni-pretraining epoches", default=5, type="int")
    ...
    parser.add_option("--anno_percent", dest="anno_percent", help="data percent", default=10, type="int")
    ...
    parser.add_option("--unmasked", dest="unmasked", action="store_true", default=False, help="use unmasked ABSA dataset")
...
def main(args):
    ...
    for dataset in args.dataset_list:
        dataset_train_list.append(
            dict_dataloarder[dataset](file_path = datasets_config[dataset]['data_dir'], split='train', aspects_list = datasets_config[dataset]['aspect_list'], annotation_percent=args.anno_percent)
        )
        dataset_val_list.append(
            dict_dataloarder[dataset](file_path = datasets_config[dataset]['data_dir'], split='val', aspects_list = datasets_config[dataset]['aspect_list'], annotation_percent=args.anno_percent)
        )
        dataset_test_list.append(
            dict_dataloarder[dataset](file_path = datasets_config[dataset]['data_dir'], split='test', aspects_list = datasets_config[dataset]['aspect_list'], annotation_percent=args.anno_percent)
        )
